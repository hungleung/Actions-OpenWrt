...
sed -i  -e 's/luci-app-ddns //' -e 's/ luci-app-autoreboot luci-app-webadmin//' -e 's/luci-app-filetransfer luci-app-vsftpd //' -e 's/ luci-app-unblockmusic//' -e 's/luci-app-arpbind //' -e 's/ luci-app-wol luci-app-ramfree//'  -e 's/luci-app-sfe luci-app-nlbwmon luci-app-accesscontrol luci-app-cpufreq/luci-app-udpxy/' -e 's/ddns-scripts_aliyun ddns-scripts_dnspod/luci-app-wireguard/' include/target.mk
